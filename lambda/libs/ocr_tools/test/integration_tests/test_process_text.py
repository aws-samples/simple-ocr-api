...
def test_process_text_img_input(test_sample_1):
    """Verify a non-empty string is returned when process_text() is called with an image file.

    Args:
        test_sample_1 (bytes): an image file encoded in bytes"""
    text = ocr_tools.process_text(test_sample_1)
    assert type(text) == str
    assert len(text) != 0


def test_process_text_pdf_input(test_sample_2):
    """Verify a non-empty string is returned when process_text() is called with an PDF file.

    Args:
        test_sample_2 (bytes): a PDF file encoded in bytes"""
    text = ocr_tools.process_text(test_sample_2)
    assert type(text) == str
    assert len(text) != 0


def test_process_text_blank_input(test_sample_3):
    """Verify no content is extracted when process_text() is called with an empty file.

    Args:
        test_sample_3 (bytes): a blank image file encoded in bytes"""
    text = ocr_tools.process_text(test_sample_3)
    assert type(text) == str
    assert len(text) == 0
